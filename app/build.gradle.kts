...
android {
    ...
    // S24 ULTRA EXCLUSIVE: ARM64 only for Snapdragon 8 Gen 3
    ...
    defaultConfig {
        ...
        externalNativeBuild {
            cmake {
                // S24 ULTRA TURBO: Maximum performance native build
    ...
    buildTypes {
        ...
        getByName("release") {
            ...
            // Minification disabled - R8 incompatible with Rikka Refine hidden API library
            // Native code is still optimized via CMake flags
            proguardFiles(getDefaultProguardFile("proguard-android-optimize.txt"), "proguard-rules.pro")
            isMinifyEnabled = false
            isShrinkResources = false
            signingConfig = signingConfigs.getByName("debug")

            // Additional flagship optimizations
            ndk {
    ...
    productFlavors {
        ...
        create("rootless") {
            ...
            manifestPlaceholders["label"] = "DSP ULTRA"
            applicationId = "me.timschneeberger.rootlessjamesdsp"  // Keep original for google-services
            // S24 Ultra Edition - SDK 35 - Snapdragon 8 Gen 3 for Galaxy
            minSdk = AndroidConfig.minSdk
            buildConfigField("boolean", "ROOTLESS", "true")
            buildConfigField("boolean", "PLUGIN", "false")
            buildConfigField("boolean", "S24_ULTRA_EDITION", "true")
        }
        create("root") {
            dimension = "version"
    ...
    buildFeatures {
        viewBinding = true
        buildConfig = true  // Fix deprecation warning
    ...
    packaging {
        jniLibs {
            // Keep all QNN libraries uncompressed for faster loading
...
dependencies {
    ...
    // ONNX Runtime with QNN (Qualcomm NPU) support
