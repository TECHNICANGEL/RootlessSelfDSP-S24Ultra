...
class MutedAudioEffectFactory {
    ...
    fun make(sid: Int, packageName: String): AudioEffect? {
        ...
        for ((name, uuid) in supportedTypeUuids) {
            ...
            // Retry with exponential backoff for session readiness issues (Error -3)
            var lastException: Exception? = null
            var retryDelay = INITIAL_RETRY_DELAY_MS

            for (attempt in 1..MAX_RETRIES) {
                try {
                    val effect = make(name, sid, packageName)
                    if (effect != null) {
                        if (attempt > 1) {
                            Timber.i("Successfully attached $name effect to session $sid on attempt $attempt")
                        }
                        return effect
                    }
                }
                    ...
                    // Error -3 typically means session not ready, worth retrying
                    val isRetryable = errorMsg.contains("-3") ||
                                      errorMsg.contains("not ready") ||
                                      ex is IllegalStateException

                    if (isRetryable && attempt < MAX_RETRIES) {
                        Timber.w("Attempt $attempt failed for $name effect on session $sid, retrying in ${retryDelay}ms...")
                        try {
                        ...
                        Timber.e("Failed to attach $name effect to session $sid (pkg: $packageName; attempt: $attempt; message: ${ex.message})")
                    }
                }
            }

            // If we get here with an exception, the effect type failed after all retries
            if (lastException != null) {
                Timber.w("All $MAX_RETRIES attempts failed for $name effect, trying next effect type...")
            }
        }
        return null
    }
            ...
            muteEffect = when (type) {
                ...
                MuteEffects.Volume -> {
                    with(AudioEffectHidden(
                        ...
                        Refine.unsafeCast<AudioEffect>(this)
                    }
                }
            }
        }
