    ...
    BroadcastReceiver(), MediaSessionManager.OnActiveSessionsChangedListener, KoinComponent
{
    ...
    // Polling job - use IO dispatcher for heavy dump operations, switch to Main for callbacks
    ...
    open fun destroy()
    {
        ...
        // MEMORY LEAK FIX: Unregister SharedPreferences listener to prevent Context leak
    ...
    private suspend fun pollSessionDump(blocking: Boolean = true)
    {
        ...
        pollingMutex.withLock {
            // Dump sessions on IO thread (heavy operation)
            val sessionDump = dumpManager.dumpSessions()
            // Process results on main thread for UI safety
            kotlinx.coroutines.withContext(Dispatchers.Main) {
                handleSessionDump(sessionDump)
            }
        }
    }
