...
// Sub-harmonic generator - creates harmonics below the fundamental
static inline float generateSubHarmonic(float input, float *state, float level)
{
	// Simple half-wave rectifier based sub-harmonic generator
	// State[0] = previous sample, State[1] = zero-crossing phase
	float output = 0.0f;

	// Detect zero crossing (positive going)
	if (input > 0.0f && state[0] <= 0.0f) {
		state[1] = 1.0f - state[1];  // Toggle phase
	}

	// Generate sub-harmonic (one octave below)
	if (state[1] > 0.5f) {
		output = input * level;
	} else {
		output = -input * level;
	...
	state[0] = input;
	return output * 0.5f;  // Reduce level to avoid clipping
}
...
void BassBoostConstructor(JamesDSPLib *jdsp)
{
	...
	jdsp->dbb.manualMode = 0;
	jdsp->dbb.manualFreq = 80.0f;
	jdsp->dbb.resonance = 2.0f;  // Q value (0.5-2.0), default matches original resonanceToQ(0.75)
	jdsp->dbb.harmonicGen = 0;
	jdsp->dbb.harmonicLevel = 0.3f;
	jdsp->dbb.subHarmState[0][0] = 0.0f;
	jdsp->dbb.subHarmState[0][1] = 0.0f;
	jdsp->dbb.subHarmState[1][0] = 0.0f;
	jdsp->dbb.subHarmState[1][1] = 0.0f;
}
...
void BassBoostSetManualMode(JamesDSPLib *jdsp, int enable)
{
	jdsp->dbb.manualMode = enable ? 1 : 0;
}

void BassBoostSetManualFreq(JamesDSPLib *jdsp, float freq)
{
	// Clamp frequency between 20 and 200 Hz
	if (freq < 20.0f) freq = 20.0f;
	if (freq > 200.0f) freq = 200.0f;
	jdsp->dbb.manualFreq = freq;
}

void BassBoostSetResonance(JamesDSPLib *jdsp, float resonance)
{
	// Clamp resonance between 0.5 and 2.0
	if (resonance < 0.5f) resonance = 0.5f;
	if (resonance > 2.0f) resonance = 2.0f;
	jdsp->dbb.resonance = resonance;
}

void BassBoostSetHarmonicGen(JamesDSPLib *jdsp, int enable, float level)
{
	jdsp->dbb.harmonicGen = enable ? 1 : 0;
	// Clamp level between 0.0 and 1.0
	if (level < 0.0f) level = 0.0f;
	if (level > 1.0f) level = 1.0f;
	jdsp->dbb.harmonicLevel = level;
}

// Enhanced DBB process with manual mode and harmonic generation
static void DBBProcessEnhanced(DBB *dbb, float *x1, float *x2, float *y1, float *y2, size_t n)
{
	int pos = dbb->downsamplerPos;

	// If in manual mode, use fixed frequency instead of FFT detection
	if (dbb->manualMode) {
		// Update filter with manual frequency directly
		// Use resonance directly as Q value (0.5-2.0 range)
		refreshStateVariable2ndOrder(&dbb->svf[0], dbb->fs, dbb->manualFreq,
									  (double)dbb->resonance, db2mag(dbb->boostdB));
	}
	...
	for (size_t framePos = 0; framePos < n; framePos++)
	{
		...
		if (pos == dbb->downsampler.factor)
		{
			...
			// Only update frequency tracking in auto mode
			if (!dbb->manualMode) {
				dbb->smoothMaxFreq = currentMaxFreq * dbb->maxSmoothingFactor + dbb->smoothMaxFreq * dbb->minusmaxSmoothingFactor;
				// Use resonance directly as Q value (0.5-2.0 range)
				refreshStateVariable2ndOrder(&dbb->svf[0], dbb->fs, dbb->smoothMaxFreq,
											  (double)dbb->resonance, db2mag(dbb->boostdB));
			}
			pos = 0;
		}
	...
	// Process with optional harmonic generation
	for (size_t i = 0; i < n; i++) {
		float delayedL = integerDelayLineProcess(&dbb->dL[0], x1[i]);
		float delayedR = integerDelayLineProcess(&dbb->dL[1], x2[i]);

		ProcessStateVariable2ndOrderStereo(&dbb->svf[0], &dbb->svf[1],
										   delayedL, delayedR, &y1[i], &y2[i]);

		// Add sub-harmonic generation if enabled
		if (dbb->harmonicGen) {
			float subL = generateSubHarmonic(y1[i] - delayedL, dbb->subHarmState[0], dbb->harmonicLevel);
			float subR = generateSubHarmonic(y2[i] - delayedR, dbb->subHarmState[1], dbb->harmonicLevel);
			y1[i] += subL;
			y2[i] += subR;
		}
	}
...
{
	DBBProcessEnhanced(&jdsp->dbb, jdsp->tmpBuffer[0], jdsp->tmpBuffer[1], jdsp->tmpBuffer[0], jdsp->tmpBuffer[1], n);
}