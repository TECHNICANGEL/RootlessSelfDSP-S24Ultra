...
abstract class BaseEqualizerSurface(
    ...
    // PERFORMANCE: Cache formatted strings to avoid allocations in onDraw()
    private val freqTextCache = mutableMapOf<Double, String>()
    private val gainTextCache = mutableMapOf<Double, String>()

    // PERFORMANCE: Get cached frequency text, formatting only on first access
    private fun getCachedFreqText(freq: Double): String {
        return freqTextCache.getOrPut(freq) { freq.prettyNumberFormat() }
    }

    // PERFORMANCE: Get cached gain text, formatting only when value changes
    private fun getCachedGainText(gain: Double): String {
        // Round to 1 decimal place for cache key to avoid excessive entries
        val roundedGain = (gain * 10).toLong() / 10.0
        return gainTextCache.getOrPut(roundedGain) { String.format(Locale.ROOT, "%.1f", gain) }
    }
    ...
    override fun onAttachedToWindow() {
        ...
        // Note: Removed forced LAYER_TYPE_HARDWARE - it can cause VulkanSurface errors
        // during surface transitions. Default LAYER_TYPE_NONE is sufficient.
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (i in mLevels.indices) {
            ...
            // PERFORMANCE: Use cached strings to avoid allocations in hot path
            canvas.drawText(getCachedFreqText(frequencyScale[i]), x, mHeight - 16, mControlBarText)
            canvas.drawText(getCachedGainText(mLevels[i]), x, mControlBarText.textSize + 8, mControlBarText)
        }
