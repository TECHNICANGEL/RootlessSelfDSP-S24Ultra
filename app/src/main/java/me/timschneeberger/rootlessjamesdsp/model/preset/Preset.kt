...
class Preset(val name: String, externalPath: File? = null): KoinComponent {
    ...
    fun validate(): Boolean {
        // PERFORMANCE: Use .use{} to ensure FileInputStream is closed
        return FileInputStream(file()).use { validate(it) }
    }
    ...
    fun load(): PresetMetadata {
        ...
        // PERFORMANCE: Use .use{} to ensure FileInputStream is closed
        return FileInputStream(file).use { load(ctx, it) }
    }
    ...
    companion object {
        ...
        private fun findLiveprogScriptPath(ctx: Context): String? {
            ...
            try {
                ...
                // PERFORMANCE: Use .use{} to ensure FileInputStream is closed
                val doc = FileInputStream(xmlFile).use { builder.parse(it) }
