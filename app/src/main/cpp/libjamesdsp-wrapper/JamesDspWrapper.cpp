...
Java_me_timschneeberger_rootlessjamesdsp_interop_JamesDspWrapper_alloc(JNIEnv *env, jobject obj, jobject callback)
{
    ...
    // PERFORMANCE: Initialize with 512 frames to match minimum enforced buffer size
    // This reduces internal reallocation when processing larger buffers
    JamesDSPInit(_dsp, 512, 48000);
...
Java_me_timschneeberger_rootlessjamesdsp_interop_JamesDspWrapper_processInt16(JNIEnv *env, jobject obj, jlong self, jshortArray inputObj, jshortArray outputObj, jint offset, jint size)
{
    ...
    // PERFORMANCE: Use GetPrimitiveArrayCritical for zero-copy access when possible
    // This avoids memory copy overhead on the hot audio processing path
    jboolean inputIsCopy = JNI_FALSE;
    jboolean outputIsCopy = JNI_FALSE;
    auto input = static_cast<jshort*>(env->GetPrimitiveArrayCritical(inputObj, &inputIsCopy));
    auto output = static_cast<jshort*>(env->GetPrimitiveArrayCritical(outputObj, &outputIsCopy));

    if (input != nullptr && output != nullptr) {
        dsp->processInt16Multiplexd(dsp, input + offset, output, inputLength / 2);
    }

    // Release in reverse order - critical sections should be as short as possible
    if (output != nullptr) env->ReleasePrimitiveArrayCritical(outputObj, output, 0);
    if (input != nullptr) env->ReleasePrimitiveArrayCritical(inputObj, input, JNI_ABORT);
}
...
Java_me_timschneeberger_rootlessjamesdsp_interop_JamesDspWrapper_processInt32(JNIEnv *env, jobject obj, jlong self, jintArray inputObj, jintArray outputObj, jint offset, jint size)
{
    ...
    // PERFORMANCE: Use GetPrimitiveArrayCritical for zero-copy access
    jboolean inputIsCopy = JNI_FALSE;
    jboolean outputIsCopy = JNI_FALSE;
    auto input = static_cast<jint*>(env->GetPrimitiveArrayCritical(inputObj, &inputIsCopy));
    auto output = static_cast<jint*>(env->GetPrimitiveArrayCritical(outputObj, &outputIsCopy));
...
Java_me_timschneeberger_rootlessjamesdsp_interop_JamesDspWrapper_processFloat(JNIEnv *env, jobject obj, jlong self, jfloatArray inputObj, jfloatArray outputObj, jint offset, jint size)
{
    ...
    jboolean inputIsCopy = JNI_FALSE;
    jboolean outputIsCopy = JNI_FALSE;
    auto input = static_cast<jfloat*>(env->GetPrimitiveArrayCritical(inputObj, &inputIsCopy));
    auto output = static_cast<jfloat*>(env->GetPrimitiveArrayCritical(outputObj, &outputIsCopy));
...
extern "C" JNIEXPORT jboolean JNICALL
Java_me_timschneeberger_rootlessjamesdsp_interop_JamesDspWrapper_setReverbCustom(JNIEnv *env, jobject obj, jlong self,
                                                                                 jboolean enable, jfloat roomSize, jfloat decay,
                                                                                 jfloat damping, jfloat diffusion, jfloat predelay,
                                                                                 jfloat wetLevel, jfloat dryLevel, jfloat width)
{
    DECLARE_DSP_B
    if(enable)
    {
        Reverb_SetCustomParam(dsp, roomSize, decay, damping, diffusion, predelay, wetLevel, dryLevel, width);
        ReverbEnable(dsp);
    }
    else
    {
...
extern "C" JNIEXPORT jboolean JNICALL
Java_me_timschneeberger_rootlessjamesdsp_interop_JamesDspWrapper_setBassBoostEnhanced(JNIEnv *env, jobject obj, jlong self,
                                                                                      jboolean enable, jfloat maxGain, jboolean manualMode,
                                                                                      jfloat manualFreq, jfloat resonance,
                                                                                      jboolean harmonicGen, jfloat harmonicLevel)
{
    DECLARE_DSP_B
    if(enable)
    {
        BassBoostSetParam(dsp, maxGain);
        BassBoostSetManualMode(dsp, manualMode ? 1 : 0);
        BassBoostSetManualFreq(dsp, manualFreq);
        BassBoostSetResonance(dsp, resonance);
        BassBoostSetHarmonicGen(dsp, harmonicGen ? 1 : 0, harmonicLevel);
        BassBoostEnable(dsp);
    }
    else
    {
...
extern "C" JNIEXPORT jboolean JNICALL
Java_me_timschneeberger_rootlessjamesdsp_interop_JamesDspWrapper_setStereoEnhancementEnhanced(JNIEnv *env, jobject obj, jlong self,
                                                                                              jboolean enable, jfloat level, jfloat depth,
                                                                                              jfloat centerLevel, jfloat lowMix,
                                                                                              jfloat midMix, jfloat highMix)
{
    DECLARE_DSP_B
    StereoEnhancementDisable(dsp);
    StereoEnhancementSetParam(dsp, level / 100.0f);
    StereoEnhancementSetDepth(dsp, depth);
    StereoEnhancementSetCenterLevel(dsp, centerLevel);
    StereoEnhancementSetBandMix(dsp, lowMix, midMix, highMix);
    if(enable)
    {
        StereoEnhancementEnable(dsp);
    }
...
Java_me_timschneeberger_rootlessjamesdsp_interop_JamesDspWrapper_setVacuumTube(JNIEnv *env, jobject obj, jlong self,
                                                                              jboolean enable, jfloat drive, jfloat bias,
                                                                              jfloat mix, jfloat evenHarm, jfloat oddHarm)
{
    DECLARE_DSP_B
    if(enable)
    {
        VacuumTubeSetDrive(dsp, drive / 100.0f);
        VacuumTubeSetBias(dsp, (bias - 50.0f) / 50.0f);  // Convert 0-100 to -1 to 1
        VacuumTubeSetMix(dsp, mix / 100.0f);
        VacuumTubeSetEvenHarm(dsp, evenHarm / 100.0f);
        VacuumTubeSetOddHarm(dsp, oddHarm / 100.0f);
...
void receiveLiveprogStdOut(const char *buffer, void* userData)
{
    ...
    // MEMORY LEAK FIX: Delete LocalRef after callback to prevent JNI reference leak
    jstring jBuffer = self->env->NewStringUTF(buffer);
    self->env->CallVoidMethod(self->callbackInterface, self->callbackOnLiveprogOutput, jBuffer);
    self->env->DeleteLocalRef(jBuffer);
}
