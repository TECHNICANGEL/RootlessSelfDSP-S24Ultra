...
abstract class BaseActivity :
    ...
    // PERFORMANCE: Cache exclusion state to avoid redundant updates
    private var lastExcludeFromRecents: Boolean? = null

    override fun onResume() {
        super.onResume()

        // PERFORMANCE: Only update appTasks when the exclusion setting actually changed
        val excludeFromRecents = prefsApp.get<Boolean>(R.string.key_exclude_app_from_recents)
        if (lastExcludeFromRecents != excludeFromRecents) {
            lastExcludeFromRecents = excludeFromRecents
            getSystemService<ActivityManager>()?.appTasks?.firstOrNull()?.setExcludeFromRecents(excludeFromRecents)
        }
    }
