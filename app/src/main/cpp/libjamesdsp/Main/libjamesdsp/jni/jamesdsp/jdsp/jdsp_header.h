...
typedef struct
{
	...
	// Tube parameters
	float drive;        // Saturation amount (0.0 - 1.0)
	float bias;         // Asymmetry for even harmonics (-1.0 to 1.0)
	float mix;          // Dry/wet blend (0.0 - 1.0)
	float evenHarm;     // Even harmonic level (2nd, 4th) - warmth
	float oddHarm;      // Odd harmonic level (3rd, 5th) - grit
	float inputGain;    // Pre-saturation gain
	float outputGain;   // Post-saturation makeup gain
	// DC blocker (highpass at ~10Hz)
	double dcBlockState[2][2];
	// High frequency softening (lowpass)
	double hfSoftState[2][2];
	double hfSoftFreq;
	// Sample rate
	double fs;
} VacuumTube;
...
typedef struct
{
	...
	// Enhanced parameters
	int manualMode;           // 0=auto (FFT detection), 1=manual frequency
	float manualFreq;         // Manual target frequency (20-200 Hz)
	float resonance;          // Q factor (0.5-2.0)
	int harmonicGen;          // Enable sub-bass harmonic generator
	float harmonicLevel;      // Harmonic generator level (0.0-1.0)
	float subHarmState[2][2]; // State for sub-harmonic generation
} DBB;
...
typedef struct
{
	...
	// Enhanced parameters
	float depth;              // Stereo depth/intensity (0.5-3.0)
	float centerLevel;        // Center channel preservation (0.0-1.0)
	float lowFreqMix;         // Bass stereo width (0.0-1.0) - keep mono for tight bass
	float midFreqMix;         // Mid frequency width (0.0-1.0)
	float highFreqMix;        // High frequency width (0.0-1.0)
} stereoEnhancement;
typedef struct
{
	NSEEL_VMCTX vm;
...
extern void BassBoostSetManualMode(JamesDSPLib *jdsp, int enable);
extern void BassBoostSetManualFreq(JamesDSPLib *jdsp, float freq);
extern void BassBoostSetResonance(JamesDSPLib *jdsp, float resonance);
extern void BassBoostSetHarmonicGen(JamesDSPLib *jdsp, int enable, float level);
extern void BassBoostProcess(JamesDSPLib *jdsp, size_t n);
// Reverb
extern void Reverb_SetParam(JamesDSPLib *jdsp, int presets);
extern void Reverb_SetCustomParam(JamesDSPLib *jdsp, float roomSize, float decay, float damping, float diffusion, float predelay, float wetLevel, float dryLevel, float width);
...
extern void StereoEnhancementSetDepth(JamesDSPLib *jdsp, float depth);
extern void StereoEnhancementSetCenterLevel(JamesDSPLib *jdsp, float level);
extern void StereoEnhancementSetBandMix(JamesDSPLib *jdsp, float lowMix, float midMix, float highMix);
...
// Vacuum tube (Analog Modeling)
extern void VacuumTubeEnable(JamesDSPLib *jdsp);
extern void VacuumTubeDisable(JamesDSPLib *jdsp);
extern void VacuumTubeSetDrive(JamesDSPLib *jdsp, float drive);
extern void VacuumTubeSetBias(JamesDSPLib *jdsp, float bias);
extern void VacuumTubeSetMix(JamesDSPLib *jdsp, float mix);
extern void VacuumTubeSetEvenHarm(JamesDSPLib *jdsp, float level);
extern void VacuumTubeSetOddHarm(JamesDSPLib *jdsp, float level);
...
// ============================================================================
// S24 ULTRA - ADVANCED DYNAMICS
// ============================================================================

// EBU R128 Loudness Normalization
typedef struct LoudnessNormalizer LoudnessNormalizer;
extern void LoudnessNormalizerInit(LoudnessNormalizer *ln, double fs);
extern void LoudnessNormalizerSetTarget(LoudnessNormalizer *ln, double targetLUFS, double truePeakLimit);
extern void LoudnessNormalizerProcess(LoudnessNormalizer *ln, float *left, float *right, size_t n);
extern void LoudnessNormalizerEnable(LoudnessNormalizer *ln);
extern void LoudnessNormalizerDisable(LoudnessNormalizer *ln);

// Multi-band Compressor (4 bands)
typedef struct MultibandCompressor MultibandCompressor;
extern void MultibandCompressorInit(MultibandCompressor *mbc, double fs);
extern void MultibandCompressorSetBand(MultibandCompressor *mbc, int band, float threshold, float ratio, float attack, float release, float makeupGain, float knee);
extern void MultibandCompressorProcess(MultibandCompressor *mbc, float *left, float *right, size_t n);
extern void MultibandCompressorEnable(MultibandCompressor *mbc);
extern void MultibandCompressorDisable(MultibandCompressor *mbc);

// Noise Gate
typedef struct NoiseGate NoiseGate;
extern void NoiseGateInit(NoiseGate *ng, double fs);
extern void NoiseGateSetParams(NoiseGate *ng, float threshold, float attack, float hold, float release, float range, float hysteresis);
extern void NoiseGateProcess(NoiseGate *ng, float *left, float *right, size_t n);
extern void NoiseGateEnable(NoiseGate *ng);
extern void NoiseGateDisable(NoiseGate *ng);

// Psychoacoustic Bass Enhancement
typedef struct PsychoacousticBass PsychoacousticBass;
extern void PsychoacousticBassInit(PsychoacousticBass *pb, double fs);
extern void PsychoacousticBassSetParams(PsychoacousticBass *pb, float frequency, float harmonicLevel, float thirdHarmonic, float fourthHarmonic, float subBassLevel, float drive, float mix);
extern void PsychoacousticBassProcess(PsychoacousticBass *pb, float *left, float *right, size_t n);
extern void PsychoacousticBassEnable(PsychoacousticBass *pb);
extern void PsychoacousticBassDisable(PsychoacousticBass *pb);
