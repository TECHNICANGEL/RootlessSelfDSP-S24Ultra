...
import me.timschneeberger.rootlessjamesdsp.fragment.DspUltraFragment
...
class MainActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // PERFORMANCE: Run asset installation on IO thread to avoid blocking main thread on first boot
        // For non-first-boot, this is a no-op so we can safely run it async
        if (firstBoot) {
            CoroutineScope(Dispatchers.IO).launch {
                assets.installPrivateAssets(this@MainActivity, force = true)
            }
        }
        ...
        // Load main fragment - S24 ULTRA uses DspUltraFragment for visualization
        dspFragment = DspFragment.newInstance()
        if(!hasLoadFailed) {
            if (isRootless()) {
                // S24 ULTRA: Use professional dashboard with visualizers
                supportFragmentManager.beginTransaction()
                    .replace(R.id.dsp_fragment_container, DspUltraFragment.newInstance())
                    .commit()
            } else {
                supportFragmentManager.beginTransaction()
    ...
    override fun onPause() {
        // Set preference BEFORE super.onPause() to give the service time to clear
        // visualization callbacks before fragment lifecycle methods are called.
        // This helps prevent HWUI surface errors by stopping visualization at the source.
    ...
    private fun showAndroid15Alert() {
        // S24 ULTRA: minSDK 35 = always Android 15+
        if(!isRootless())
            return
        ...
        MaterialAlertDialogBuilder(this)
            ...
            .setNegativeButton(R.string.close) { _, _ ->
                prefsVar.set(R.string.key_android15_screenrecord_restriction_seen, checkBox.isChecked)
                // Auto-dismissed by MaterialAlertDialogBuilder
            }
            .show()
    }
    ...
    private fun dismissUpdate() {
        ...
        MaterialAlertDialogBuilder(this)
            ...
            .setItems(R.array.update_dismiss_dialog) { dialogInterface, i ->
                ...
                // Delay dismiss to allow SurfaceFlinger buffers to drain
                android.os.Handler(android.os.Looper.getMainLooper()).postDelayed({
                    try { dialogInterface.dismiss() } catch (_: Exception) {}
                }, 50)
            }
    ...
    fun requestCapturePermission() {
        // S24 ULTRA: Android 15+ (API 35) always requires new permission
        // Token reuse not allowed, always request new
        if(false) { // Disabled token reuse for Android 15+
            binding.powerToggle.isToggled = true
    ...
    private fun quitGracefully() {
        ...
        // PERFORMANCE: Use coroutines instead of Timer to avoid creating new threads
