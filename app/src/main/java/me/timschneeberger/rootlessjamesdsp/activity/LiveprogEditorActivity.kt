...
class LiveprogEditorActivity : BaseActivity() {
    ...
    private fun run() {
        ...
        // PERFORMANCE: Use MODE_PRIVATE instead of deprecated MODE_MULTI_PROCESS (crashes on API 35+)
        val liveprog = getSharedPreferences(Constants.PREF_LIVEPROG, Context.MODE_PRIVATE)
        ...
        if(!liveprog.getBoolean(key, false)) {
            ...
            // PERFORMANCE: Use apply() instead of commit() to avoid blocking main thread
            liveprog.edit().putBoolean(key, true).apply()
        }
        ...
        if(liveprog.getString(key, "") != path) {
            // PERFORMANCE: Use apply() instead of commit() to avoid blocking main thread
            liveprog.edit().putString(key, path).apply()
        }
