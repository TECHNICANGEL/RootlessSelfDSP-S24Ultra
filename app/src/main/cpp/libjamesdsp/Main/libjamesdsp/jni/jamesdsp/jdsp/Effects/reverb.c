...
// Custom reverb with individual parameter control
// roomSize: 0.0-1.0 (small to large room)
// decay: 0.5-10.0 seconds (RT60)
// damping: 0.0-1.0 (0=bright, 1=dark/absorbed highs)
// diffusion: 0.0-1.0 (echo density)
// predelay: 0-100ms
// wetLevel: -70 to 0 dB
// dryLevel: -70 to 0 dB
// width: 0.0-1.0 (stereo spread)
void Reverb_SetCustomParam(JamesDSPLib *jdsp, float roomSize, float decay, float damping,
                           float diffusion, float predelay, float wetLevel, float dryLevel, float width)
{
	int rate = (int)jdsp->fs;

	// Clamp inputs
	roomSize = clampf(roomSize, 0.0f, 1.0f);
	decay = clampf(decay, 0.5f, 10.0f);
	damping = clampf(damping, 0.0f, 1.0f);
	diffusion = clampf(diffusion, 0.0f, 1.0f);
	predelay = clampf(predelay, 0.0f, 0.1f);  // max 100ms
	wetLevel = clampf(wetLevel, -70.0f, 0.0f);
	dryLevel = clampf(dryLevel, -70.0f, 0.0f);
	width = clampf(width, 0.0f, 1.0f);

	// Convert room size to early reflection parameters
	float erFactor = 0.7f + roomSize * 1.5f;  // 0.7 - 2.2
	float erToLate = 0.1f + roomSize * 0.5f;  // Early reflection mix
	float erWet = -9.0f - roomSize * 3.0f;    // Early reflection level
	float erWidth = 0.4f + width * 0.6f;      // Early reflection width

	// Convert damping to filter frequencies
	float dampLpf = 18000.0f - damping * 13000.0f;  // 5000 - 18000 Hz
	float bassLpf = 300.0f + (1.0f - damping) * 400.0f;  // Bass filter
	float inputLpf = 18000.0f - damping * 4000.0f;
	float outputLpf = 18000.0f - damping * 8000.0f;

	// Diffusion affects wander (modulation depth) and bass boost
	float wander = 0.15f + diffusion * 0.25f;
	float bassB = 0.1f + diffusion * 0.15f;
	float spin = 0.5f + diffusion * 1.1f;  // LFO rate

	// Use oversample factor based on decay time (longer = higher quality)
	int osf = decay > 3.0f ? 2 : 1;

	sf_advancereverb(&jdsp->reverb, rate, osf,
		erToLate,      // ertolate - early reflection to late mix
		erWet,         // erefwet - early reflection wet level (dB)
		dryLevel,      // dry level (dB)
		erFactor,      // ereffactor - early reflection timing factor
		erWidth,       // erefwidth - early reflection stereo width
		width,         // width - late reverb stereo width
		wetLevel,      // wet level (dB)
		wander,        // wander - modulation depth
		bassB,         // bassb - bass boost in reverb tail
		spin,          // spin - LFO rate
		inputLpf,      // inputlpf
		bassLpf,       // basslpf
		dampLpf,       // damplpf - main damping filter
		outputLpf,     // outputlpf
		decay,         // rt60 - decay time in seconds
		predelay       // delay - pre-delay in seconds
	);
}
