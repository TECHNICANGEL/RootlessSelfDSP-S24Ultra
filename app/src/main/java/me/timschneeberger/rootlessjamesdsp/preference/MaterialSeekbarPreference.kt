...
class MaterialSeekbarPreference : Preference {
    ...
    fun  /* synthetic access */updateLabelValue(value: Float) {
        if (mSeekBarValueTextView != null) {
            ...
            {
                // Format value then append unit (avoid % in format string causing UnknownFormatConversionException)
                mSeekBarValueTextView!!.text = "%.${mPrecision}f".format(Locale.ROOT, value) + mUnit
            }
            else
            {
