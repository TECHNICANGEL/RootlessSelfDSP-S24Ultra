...
// ═══════════════════════════════════════════════════════════════
// S24 ULTRA TURBO - ARM NEON SIMD Optimizations
// ═══════════════════════════════════════════════════════════════
...
{
#if USE_NEON
    // TURBO: Optimized stereo deinterleave with NEON
    if (num_channels == 2) {
        float *left = chan_buffers[0];
        float *right = chan_buffers[1];
        unsigned int i = 0;

        // Process 8 stereo pairs (16 floats) per iteration
        for (; i + 8 <= num_frames; i += 8) {
            PREFETCH(&buffer[(i + 16) * 2]);

            // Load 8 interleaved stereo pairs
            float32x4x2_t pair0 = vld2q_f32(&buffer[i * 2]);
            float32x4x2_t pair1 = vld2q_f32(&buffer[(i + 4) * 2]);

            // Store deinterleaved
            vst1q_f32(&left[i], pair0.val[0]);
            vst1q_f32(&left[i + 4], pair1.val[0]);
            vst1q_f32(&right[i], pair0.val[1]);
            vst1q_f32(&right[i + 4], pair1.val[1]);
        }
...
#endif
    // Fallback for non-stereo or non-NEON
...
{
    unsigned int samples = num_frames * num_channels;
    union {
        int32_t raw;
        float f;
    } fltInt;
...
#if USE_NEON
    // TURBO: Fast stereo interleave with NEON, then compute CRC
    if (num_channels == 2) {
        const float *left = chan_buffers[0];
        const float *right = chan_buffers[1];
        unsigned int i = 0;

        // Fast NEON interleave - 8 frames per iteration
        for (; i + 8 <= num_frames; i += 8) {
            PREFETCH(&left[i + 16]);
            PREFETCH(&right[i + 16]);
            ...
            float32x4x2_t interleaved0 = {{ l0, r0 }};
            float32x4x2_t interleaved1 = {{ l1, r1 }};

            vst2q_f32(&buffer[i * 2], interleaved0);
            vst2q_f32(&buffer[(i + 4) * 2], interleaved1);
        }

        // Scalar remainder for interleave
        for (; i < num_frames; i++) {
            buffer[i * 2] = left[i];
            buffer[i * 2 + 1] = right[i];
        }

        // CRC32 computation (must be scalar for correctness)
        for (i = 0; i < samples; i++) {
            fltInt.f = buffer[i];
            crc = crc ^ fltInt.raw;
            for (int j = 7; j >= 0; j--) {
...
#if USE_NEON
    // TURBO: Find max with NEON (4x faster)
    for (i = 0; i < channels; i++) {
        float32x4_t vmax = vdupq_n_f32(0.0f);
        j = 0;

        // Process 16 samples per iteration
        for (; j + 16 <= nsamples; j += 16) {
            PREFETCH(&signal[i][j + 32]);

            float32x4_t v0 = vabsq_f32(vld1q_f32(&signal[i][j]));
            float32x4_t v1 = vabsq_f32(vld1q_f32(&signal[i][j + 4]));
            float32x4_t v2 = vabsq_f32(vld1q_f32(&signal[i][j + 8]));
            float32x4_t v3 = vabsq_f32(vld1q_f32(&signal[i][j + 12]));
        ...
        float channel_max = vget_lane_f32(vmax2, 0);

        // Scalar remainder
        for (; j < nsamples; j++) {
            float absval = fabsf(signal[i][j]);
            if (absval > channel_max) channel_max = absval;
        }

        if (channel_max > max) max = channel_max;
    }
#else
    // Scalar fallback
	for (i = 0; i < channels; i++)
...
#define NUMPTS 15
#define NUMPTS_DRS (7)  // Must match jdsp_header.h
