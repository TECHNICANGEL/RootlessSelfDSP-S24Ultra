...
public class SummariedListPreferenceLiveProg extends ListPreference
{
    public SummariedListPreferenceLiveProg(Context context, AttributeSet set)
    {
        super(context, set);
    }
    ...
    protected void onPrepareDialogBuilder(Builder builder)
    {
        try
        {
            if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED))
            {
                ...
                String tip = getContext().getResources().getString(R.string.text_liveprog_dir_isempty);
                tip = String.format(tip, DSPManager.liveprogPath);
                Toast.makeText(getContext(), tip, Toast.LENGTH_LONG).show();
                super.onPrepareDialogBuilder(builder);
                return;
            }
            final String kernelPath = DSPManager.liveprogPath;
            ...
            getFileNameList(kernelFile, ".eel", kernelList);
            getFileNameList(kernelFile, ".txt", kernelList);
            getFileNameList(kernelFile, ".liveprog", kernelList);
