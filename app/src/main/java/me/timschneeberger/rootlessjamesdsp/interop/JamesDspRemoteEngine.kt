...
class JamesDspRemoteEngine(
    ...
    override fun setVacuumTube(enable: Boolean, drive: Float, bias: Float, mix: Float, evenHarm: Float, oddHarm: Float): Boolean {
        var ret = true
        if (enable) {
            ret = ret and (effect.setParameter(150, (drive * 10).roundToInt().toShort()) == AudioEffect.SUCCESS)
            ret = ret and (effect.setParameter(151, (bias * 10).roundToInt().toShort()) == AudioEffect.SUCCESS)
            ret = ret and (effect.setParameter(152, (mix * 10).roundToInt().toShort()) == AudioEffect.SUCCESS)
            ret = ret and (effect.setParameter(153, (evenHarm * 10).roundToInt().toShort()) == AudioEffect.SUCCESS)
            ret = ret and (effect.setParameter(154, (oddHarm * 10).roundToInt().toShort()) == AudioEffect.SUCCESS)
        }
