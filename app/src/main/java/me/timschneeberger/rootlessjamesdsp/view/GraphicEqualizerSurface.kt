...
class GraphicEqualizerSurface(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
    ...
    // PERFORMANCE: Cache text width measurements to avoid expensive measureText() in onDraw
    private val textWidthCache = mutableMapOf<String, Float>()

    // PERFORMANCE: Get cached text width, measuring only on first access
    private fun getCachedTextWidth(text: String): Float {
        return textWidthCache.getOrPut(text) { mControlBarText.measureText(text) }
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (i in mGains.indices) {
            ...
            if (mGains.size > 1 /* only if more than 1 nodes defined */ &&
                ...
                val freqWidth = getCachedTextWidth(freqText)  // PERFORMANCE: Use cached width
                val gainText = String.format(Locale.ROOT, "%.1f", mGains[i])
                val gainWidth = getCachedTextWidth(gainText)  // PERFORMANCE: Use cached width
